...
namespace IPUC.AA.Back.BL.Implementations
{
    public class PaymentBL : IPaymentBL
    {
        ...
        public async Task<PaymentTotalModel> GetAllTotals()
        {
            NumberFormatInfo ci = new CultureInfo("es-CO", false).NumberFormat;
            var entities = await _paymentBD.GetAllPaymentsAsync();            
            Int64 debits = entities.Sum(x => x.Value);
            var users = entities.GroupBy(x => x.UserId).Select(item => new 
            {
                item.First().User.CampSpace,
                item.First().User.TypeTransportId
            }).ToList();
            ...
            {
                CampSpace = campSpaceValue.ToString(),
                Car = totalCar.ToString(),
                Bus = totalBus.ToString(),
                Credits = (valueTotal- debits).ToString("C", ci),
                Debits = debits.ToString("C", ci),
                PaymentsTotal = valueTotal.ToString("C", ci)
            };
        }
    }
}
